...
app.post("/cat/add", (req, res) => {
  ...
  if (name && !data.categories[name]) {
    data.categories = { [name]: [], ...data.categories };
    saveData();
  }
  res.sendStatus(200);
});
...
app.post("/cat/rename", (req, res) => {
  ...
  if (data.categories[oldName] && newName?.trim()) {
    data.categories[newName.trim()] = data.categories[oldName];
...
textarea,input{width:100%;padding:8px;margin:4px 0;border-radius:6px;border:1px solid #333;background:#121212;color:#eee}
.item{border-bottom:1px solid #333;padding:8px;margin-bottom:4px}
#quickBtns button{margin-right:6px;}
#notes input{width:70%;margin-right:6px;}
</style>
</head>
<body>
...
function render(){
  ...
  Object.keys(state.categories||{}).forEach(c=>{
    ...
    const btn=document.createElement("button"); btn.textContent=c; btn.onclick=()=>openCat(c);
    const rename=document.createElement("button"); rename.textContent="✏️"; rename.onclick=()=>{ const n=prompt("Neuer Name",c); if(n) renameCat(c,n);}
    const del=document.createElement("button"); del.textContent="🗑"; del.onclick=()=>{ if(confirm("Kategorie löschen?")) delCat(c);}
    item.appendChild(btn); item.appendChild(rename); item.appendChild(del);
    catsDiv.appendChild(item);
  });

  // Aktive Kategorie Notizen
  const catNotes=document.getElementById("catNotes"); catNotes.innerHTML="";
  if(activeCat && state.categories[activeCat]){
    const notes=state.categories[activeCat];
    const div=document.createElement("div");
    const inp=document.createElement("input"); inp.id="newNote"; inp.placeholder="Neue Notiz...";
    const btn=document.createElement("button"); btn.textContent="Speichern"; btn.onclick=addNote;
    div.appendChild(inp); div.appendChild(btn); catNotes.appendChild(div);
    notes.forEach((n,i)=>{
      const d=document.createElement("div"); d.className="item"; d.textContent=n.text;
      const del=document.createElement("button"); del.textContent="🗑"; del.onclick=()=>delNote(i);
      d.appendChild(del); catNotes.appendChild(d);
    });
  }
}

function editHist(i){ fetch("/history/edit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({i})}); }
function delHist(i){ fetch("/history/delete",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({i}) ); render(); }

function addCat(){ const v=document.getElementById("newCat").value.trim(); if(v) fetch("/cat/add",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:v})}); }
function delCat(n){ fetch("/cat/delete",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:n})}); render(); }
function renameCat(oldN,newN){ fetch("/cat/rename",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({oldName:oldN,newName:newN})}); render(); }

function openCat(c){ activeCat=c; render(); }
function addNote(){ const t=document.getElementById("newNote").value.trim(); if(!t) return; fetch("/note/add",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({cat:activeCat,text:t})}); document.getElementById("newNote").value=""; }
function delNote(i){ fetch("/note/delete",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({cat:activeCat,i}) ); render();}
</script>
</body>
</html>
`);
